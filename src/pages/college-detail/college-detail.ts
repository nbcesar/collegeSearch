...
export class CollegeDetailPage {
  ...
  ionViewDidLoad() {
    ...
    //this.inList = this.profileService.inList(this.collegeData['unitid']);
    this.profileService.inList(this.collegeData['unitid']).on('value', snapshot => {
      if (snapshot.exists()) this.inList = true;
